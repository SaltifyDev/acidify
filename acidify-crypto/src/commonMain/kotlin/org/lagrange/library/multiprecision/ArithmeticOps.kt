...
internal object ArithmeticOps {
    ...
    fun addMagnitudes(
        ...
        while (i < maxSize) {
            val (sum, newCarry) = addWithCarry(longer[i], 0UL, carry)
            result[i] = sum
            carry = newCarry
            i++
        }
    ...
    fun subMagnitudes(
        ...
        while (i < b.size) {
            val (diff, newBorrow) = subWithBorrow(a[i], b[i], borrow)
            result[i] = diff
            borrow = newBorrow
            i++
        }
        ...
        while (i < a.size) {
            val (diff, newBorrow) = subWithBorrow(a[i], 0UL, borrow)
            result[i] = diff
            borrow = newBorrow
            i++
        }
    ...
    fun schoolbookMultiply(
        ...
        for (i in a.indices) {
            ...
            for (j in b.indices) {
                ...
                val (r1, c1) = addWithCarry(result[i + j], low, false)
    ...
    fun karatsubaMultiply(
        ...
        for (i in z2.indices) {
            ...
            val (sum, c) = addWithCarry(result[i + half], z2[i], carry != 0UL)
            result[i + half] = sum
        ...
        for (i in z1.indices) {
            ...
            val (sum, c) = addWithCarry(result[i + 2 * half], z1[i], carry != 0UL)
            result[i + 2 * half] = sum
            carry = if (c) 1UL else 0UL
        }
    }
    ...
    private fun knuthDivide(dividend: BigInt, divisor: BigInt): Pair<BigInt, BigInt> {
        ...
        for (j in m downTo 0) {
            ...
            val temp = DWord(normDividendWords[j + n], normDividendWords[j + n - 1])
            ...
            for (k in 0 until n) {
                val (high, low) = mulWord(qHat, normDivisorWords[k])

                val (r1, b1) = subWithBorrow(normDividendWords[j + k], low, borrow)
            ...
            val (finalResult, finalBorrow) = subWithBorrow(normDividendWords[j + n], carry, borrow)
            ...
            if (borrow) {
                ...
                carry = 0UL
                for (k in 0 until n) {
                    val (sum, c) = addWithCarry(normDividendWords[j + k], normDivisorWords[k], carry != 0UL)
    ...
    fun square(result: ULongArray, a: ULongArray) {
        ...
        for (i in 0 until n) {
            ...
            for (j in 0 until i) {
                ...
                val (doubleLow, c1) = addWithCarry(low, low, false)
                val doubleHigh = (high shl 1) or (if (c1) 1UL else 0UL)

                // Add to result
                val (r, c2) = addWithCarry(result[i + j], doubleLow, carry != 0UL)
                result[i + j] = r
            ...
            val (r, c3) = addWithCarry(result[2 * i], low, carry != 0UL)
            result[2 * i] = r
            ...
            while (k < result.size && carry != 0UL) {
                val (sum, c) = addWithCarry(result[k], 0UL, carry != 0UL)
                result[k] = sum
                carry = if (c) 1UL else 0UL
                k++
            }
