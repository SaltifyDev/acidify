...
class BigInt : Comparable<BigInt> {
    ...
    fun bitLength(): Int {
        ...
        return bits + (WORD_BITS - countLeadingZeros(words.last()))
    }
    ...
    private fun addMagnitude(other: BigInt) {
        ...
        for (i in 0 until other.words.size) {
            val (result, newCarry) = addWithCarry(words[i], other.words[i], carry)
            words[i] = result
            carry = newCarry
        }
        ...
        for (i in other.words.size until maxSize) {
            if (!carry) break
            val (result, newCarry) = addWithCarry(words[i], 0UL, carry)
            words[i] = result
            carry = newCarry
        }
    ...
    private fun subMagnitude(other: BigInt) {
        ...
        for (i in 0 until other.words.size) {
            val (result, newBorrow) = subWithBorrow(words[i], other.words[i], borrow)
            words[i] = result
            borrow = newBorrow
        }
        ...
        for (i in other.words.size until words.size) {
            if (!borrow) break
            val (result, newBorrow) = subWithBorrow(words[i], 0UL, borrow)
            words[i] = result
            borrow = newBorrow
        }
