...
internal object QRCodeSetup {
    ...
    fun setupPositionProbePattern(
        ...
        val squareData = QRCodeSquare(
            ...
            rowSize = probeSize,
            colSize = probeSize,
            squareInfo = QRCodeSquareInfo(QRCodeSquareType.POSITION_PROBE, QRCodeRegion.UNKNOWN),
        ...
        for (row in -1..probeSize) {
            for (col in -1..probeSize) {
                ...
                modules[row + rowOffset][col + colOffset] = QRCodeSquare(
                    ...
                    row = row + rowOffset,
                    col = col + colOffset,
                    squareInfo = QRCodeSquareInfo(QRCodeSquareType.POSITION_PROBE, region),
                    moduleSize = modulesSize,
                    parent = squareData,
                )
            }
    ...
    private fun findSquareRegion(row: Int, col: Int, probeSize: Int) =
        when (row) {
            0 -> when (col) { // 0 x ?: ┌───┐
                0 -> QRCodeRegion.TOP_LEFT_CORNER // 0 x 0: ┌
                probeSize - 1 -> QRCodeRegion.TOP_RIGHT_CORNER // 0 x MAX: ┐
                probeSize -> QRCodeRegion.MARGIN // Outside boundaries
                else -> QRCodeRegion.TOP_MID // between: ─
            }

            probeSize - 1 -> when (col) { // MAX x ?: └───┘
                0 -> QRCodeRegion.BOTTOM_LEFT_CORNER // MAX x 0: └
                probeSize - 1 -> QRCodeRegion.BOTTOM_RIGHT_CORNER // MAX x MAX: ┘
                probeSize -> QRCodeRegion.MARGIN // Outside boundaries
                else -> QRCodeRegion.BOTTOM_MID // between: ─
            }

            probeSize -> QRCodeRegion.MARGIN // Outside boundaries

            else -> when (col) { // Inside boundaries but not in any edge
                0 -> QRCodeRegion.LEFT_MID
                probeSize - 1 -> QRCodeRegion.RIGHT_MID
                probeSize -> QRCodeRegion.MARGIN // Outside boundaries
                else -> QRCodeRegion.CENTER // Middle/Center square
            }
        }
