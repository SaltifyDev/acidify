...
inline fun addWithCarry(a: Word, b: Word, carryIn: Boolean): Pair<Word, Boolean> {
...
inline fun subWithBorrow(a: Word, b: Word, borrowIn: Boolean): Pair<Word, Boolean> {
...
inline fun mulWord(a: Word, b: Word): Pair<Word, Word> {
...
// Count leading zeros
inline fun countLeadingZeros(x: Word): Int {
    if (x == 0UL) return WORD_BITS
    return x.countLeadingZeroBits()
}

// Count trailing zeros
inline fun countTrailingZeros(x: Word): Int {
    if (x == 0UL) return WORD_BITS
    return x.countTrailingZeroBits()
}

// Constant-time comparison utilities
inline fun constantTimeEq(a: Word, b: Word): Word {
    return (a xor b).inv() + 1UL  // Returns WORD_MAX if equal, 0 otherwise
}

inline fun constantTimeSelect(mask: Word, a: Word, b: Word): Word {
